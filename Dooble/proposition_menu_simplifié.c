...
int main() {
    // Initialisation SDL + TTF (à retirer u// Aymeric : je trouve c est juste un peu moins obscure et plus clair que celui de chatGPT, a vous de me dire
...
int main() {
...
}ne fois "associé" avec le main, c est juste pour le test
    SDL_Init(SDL_INIT_EVERYTHING);
    TTF_Init();
    ...
    SDL_Surface* tittleSurface = TTF_RenderText_Solid(bigFont, "Dooble", tittleTextColor);
    SDL_Texture* tittleTexture = SDL_CreateTextureFromSurface(renderer_menu, tittleSurface);
    SDL_Rect tittleRect = {screen.w/2 - tittleSurface->w/2, screen.h/8 - tittleSurface->h/2.5, tittleSurface->w, tittleSurface->h};
    SDL_FreeSurface(tittleSurface);
    ...
    while(running) {
        ...
        // Dessiner le texte principal "Dooble"
        SDL_RenderCopy(renderer_menu, tittleTexture, NULL, &tittleRect);
